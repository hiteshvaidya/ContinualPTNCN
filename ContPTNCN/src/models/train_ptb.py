...
import numpy as np
from rnn import create_rnn_model, RNN
from ptb_data_loader import PTBDataLoader
import time
import math
...
def create_train_step_ptb(model: RNN):
    ...
    def train_step(params: dict, x_batch: jnp.ndarray, y_batch: jnp.ndarray, learning_rate: float = 0.001):
        ...
        def loss_fn(params):
            ...
            predictions, _ = model.forward_sequence(params, x_batch)
            return cross_entropy_loss(predictions, y_batch)
...
def generate_text(model: RNN, params: dict, data_loader: PTBDataLoader, 
    ...
    # Process seed text first
    if len(seed_indices) > 1:
        seed_onehot = jnp.eye(data_loader.vocab_size)[jnp.array(seed_indices[:-1])]
        seed_onehot = jnp.expand_dims(seed_onehot, axis=0)  # Add batch dimension
        _, states = model.forward_sequence(params, seed_onehot, states)
    ...
    for _ in range(length):
        ...
        current_input = jnp.eye(data_loader.vocab_size)[current_char_idx]
        current_input = jnp.expand_dims(jnp.expand_dims(current_input, axis=0), axis=0)
        
        # Forward pass for single step
        prediction, states = model.forward_step(params, current_input[:, 0, :], states)
        
        # Get logits and apply temperature
        logits = prediction[0, :] / temperature  # (vocab_size,)
...
def main():
    ...
    model = create_rnn_model(
        input_size=vocab_size,  # One-hot encoded characters
