...
import numpy as np
from ptncn_FWAndGate import PTNCN_Gate_FastWeight
...
mb = 200 #2
v_mb = 400 #32
eval_iter = float('inf') # changed to infinity to disable evaluation during training
t_prime = 1 #0

# meta-parameters for model itself --> manhattan distance
model_form = "ptncn_gate_fw"
n_e = 30 
opt_type = "nag"
init_type = "normal"
alpha = 0.1 # increased from 0.075
momentum = 0.9 # decreased from 0.95 for faster convergence
update_radius = 1.0
param_radius = -30
w_decay = -0.0001
hid_dim = 250 # reduced from 1000
wght_sd = 0.05
err_wght_sd = 0.05
beta = 0.1
gamma = 1
...
# Fast weight and gate parameters
fast_weight_lr = 0.1  # η in the paper
fast_weight_decay = 0.9  # λ in the paper
...
if eval_only is False:
    ...
    log.write("Iter, Loss, PPL, Acc, VLoss, VPPL, VAcc, TrainTime\n")
    log.flush()
    log.write("{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}\n".format(-1, cost_i, ppl_i, acc_i, vcost_i, vppl_i, vacc_i, 0.0))
    log.flush()
    ...
    for e in range(n_e):
        ...
        for x_seq in train_data:
            ...
            for t in range(x_seq.shape[1]): # step through sequence
                ...
                if t >= t_prime:
                    ...
                    if accum_updates is False:
                        ...
                        # Create new list of properly shaped gradients
                        reshaped_delta = []
                        for i, (d, p) in enumerate(zip(delta, model.param_var)):
                            # print(f"\nGradient computation for parameter {i}:")
                            # print(f"Original delta[{i}] shape: {d.shape}, total elements: {tf.size(d)}")
                            # print(f"Parameter[{i}] shape: {p.shape}, total elements: {tf.size(p)}")
                            # Ensure gradient maintains proper shape
                            if len(d.shape) == 1:
                                d = tf.reshape(d, p.shape)
                            elif d.shape != p.shape:
                                d = tf.reshape(d, p.shape)
                            reshaped_delta.append(d)
                            # print(f"Reshaped delta[{i}] shape: {d.shape}, total elements: {tf.size(d)}")
                        
                        # Apply gradients with properly shaped tensors
                        optimizer.apply_gradients(zip(reshaped_delta, model.param_var))

                        if w_decay > 0.0:
                            for p in range(len(reshaped_delta)):
                                reshaped_delta[p] = reshaped_delta[p] - (reshaped_delta[p] * w_decay)
            ...
            if tick >= eval_iter:
                ...
                log.write("{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}\n".format(e, cost_i, ppl_i, acc_i, vcost_i, vppl_i, vacc_i, eval_time_v))
                log.flush()
                norm_log.write("{0}\n".format(theta_norms_to_str(model)))
                norm_log.flush()
        ...
        if tick > 0:
            cost_i, acc_i, vcost_i, vacc_i, eval_time_v, ppl_i, vppl_i, mse_i, vmse_i = eval_model_timed(model, train_data, dev_data, subtrain_data=subtrain_data)
            print(" {0}: Tr.L = {1} Tr.Acc = {2} V.L = {3} V.Acc = {4} Tr.MSE = {5} V.MSE = {6} in {7} s".format(e, cost_i, acc_i, vcost_i, vacc_i, mse_i, vmse_i, eval_time_v))
            log.write("{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}\n".format(e, cost_i, ppl_i, acc_i, vcost_i, vppl_i, vacc_i, train_time))
            log.flush()
            norm_log.write("{0}\n".format(theta_norms_to_str(model)))
            norm_log.flush()
